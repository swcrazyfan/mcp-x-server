...
// Initialize server with authenticated user context
async function initializeServer() {
    const authenticatedUser = await getAuthenticatedUser();
    const contextInfo = authenticatedUser ? 
        `This Twitter API server is authenticated as @${authenticatedUser.username} (${authenticatedUser.name}).` : 
        'Twitter API server is authenticated with provided credentials.';
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => ({
        tools: Object.entries(TOOLS).map(([name, tool]) => {
            const toolDescription = authenticatedUser ? 
                `${tool.description} (Executing as @${authenticatedUser.username})` : 
                tool.description;
